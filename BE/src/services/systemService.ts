...
function toAccountStatus(value: unknown, fallback = AccountStatus.ACTIVE): AccountStatus {
...
export async function updateFaculty(idValue: unknown, input: any, actorId: number, ipAddress?: string) {
  ...
  const updated = await prisma.faculty.update({ where: { id }, data: { name, code } });
  ...
  const student = await prisma.$transaction(async (tx) => {
    ...
    return tx.student.create({
      data: {
        ...
        status,
        requiredWorkdays: Number(input.requiredWorkdays ?? settings?.defaultRequiredWorkdays ?? 12),
      },
    });
  });
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    return tx.student.update({
      ...
      data: {
        ...
        status,
        requiredWorkdays: input.requiredWorkdays === undefined ? existing.requiredWorkdays : Number(input.requiredWorkdays),
      },
    });
  });
...
export async function updateUser(idValue: unknown, input: any, actorId: number, ipAddress?: string) {
  ...
  if (input.password) data.passwordHash = await bcrypt.hash(String(input.password), 10);
...
export async function createActivityLog(input: any, actorId: number, ipAddress?: string) {
  const action = String(input.action ?? '').trim();
  const affectedItem = String(input.affectedItem ?? '').trim();
  if (!action || !affectedItem) throw new BusinessError(400, 'Hành động và đối tượng bị ảnh hưởng là bắt buộc');
  await writeActivityLog(actorId, action, affectedItem, input.oldValue, input.newValue, ipAddress);
}
