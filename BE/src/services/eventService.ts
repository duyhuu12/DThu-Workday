...
function validateAndNormalizeEvent(input: any) {
  ...
  const startTime = String(input.startTime || '07:00');
  const endTime = String(input.endTime || '11:00');
  ...
  return {
    ...
    startTime,
    endTime,
    shift: String(input.shift || 'morning').toUpperCase() as WorkShift,
    registrationOpen,
    registrationClose,
...
export async function getAllEvents(
  ...
  const now = new Date();
  await prisma.workEvent.updateMany({
    where: { status: EventStatus.APPROVED, registrationOpen: { lte: now }, registrationClose: { gte: now } },
...
export async function getEventDetail(id: number) {
  const event = await prisma.workEvent.findUnique({ where: { id }, include: { organizer: true } });
...
export async function openRegistration(
  ...
  const updated = await prisma.workEvent.update({
    ...
    data: {
      ...
      registrationOpen:
        existing.registrationOpen > now ? now : existing.registrationOpen,
    },
    include: { organizer: true },
  });
