...
export default function WorkCreditsPage() {
  const { credits, settings, fetchCredits, fetchCurrentStudent } = useAppStore();
  ...
  const required = Math.max(1, student?.requiredWorkdays ?? settings.defaultRequiredWorkdays); const pct = Math.min(100, Math.round((totalEarned / required) * 100));
