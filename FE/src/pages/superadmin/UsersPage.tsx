...
export default function UsersPage() {
  ...
  async function setAccountStatus(target: DisplayUser, nextStatus: 'active' | 'locked') {
    try {
      if (['student', 'classleader'].includes(target.role) && target.studentRecordId) await updateStudent(target.studentRecordId, { status: nextStatus });
  ...
  const columns: Column<DisplayUser>[] = [
    ...
    { key: 'actions', header: '', render: (u) => <div className="flex gap-1">{!['student', 'classleader'].includes(u.role) && <Button size="sm" variant="ghost" onClick={() => openEdit(u)}><Pencil className="h-4 w-4" /></Button>}{u.status === 'active' ? <Button size="sm" variant="ghost" onClick={() => setLockTarget(u)}><Lock className="h-4 w-4 text-destructive" /></Button> : <Button size="sm" variant="ghost" onClick={() => void setAccountStatus(u, 'active')}><Unlock className="h-4 w-4 text-success" /></Button>}{!['student', 'classleader'].includes(u.role) && <Button size="sm" variant="ghost" onClick={() => setDeleteTarget(u)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}</div> },
  ];
