...
export async function createSemester(req: AuthRequest, res: Response) {
  const user = requireUser(req, res); if (!user) return;
  try {
    const data = await systemService.createSemester(req.body, user.id, clientIp(req));
    res.status(201).json({ success: true, message: 'Thêm học kỳ thành công', data });
  } catch (error) { handleError(res, error, 'Lỗi hệ thống khi thêm học kỳ'); }
}

export async function updateSemester(req: AuthRequest, res: Response) {
  const user = requireUser(req, res); if (!user) return;
  try {
    const data = await systemService.updateSemester(req.params.id, req.body, user.id, clientIp(req));
    res.status(200).json({ success: true, message: 'Cập nhật học kỳ thành công', data });
  } catch (error) { handleError(res, error, 'Lỗi hệ thống khi cập nhật học kỳ'); }
}

export async function activateSemester(req: AuthRequest, res: Response) {
  const user = requireUser(req, res); if (!user) return;
  try {
    const data = await systemService.activateSemester(req.params.id, user.id, clientIp(req));
    res.status(200).json({ success: true, message: 'Đã chọn học kỳ hiện tại', data });
  } catch (error) { handleError(res, error, 'Lỗi hệ thống khi chọn học kỳ'); }
}

export async function deleteSemester(req: AuthRequest, res: Response) {
  const user = requireUser(req, res); if (!user) return;
  try {
    await systemService.deleteSemester(req.params.id, user.id, clientIp(req));
    res.status(200).json({ success: true, message: 'Xóa học kỳ thành công' });
  } catch (error) { handleError(res, error, 'Lỗi hệ thống khi xóa học kỳ'); }
}
