...
export default function SettingsPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast({ title: 'Đã lưu cài đặt vào database' });
    } catch (error) {
      toast({ title: 'Không thể lưu cài đặt', description: error instanceof Error ? error.message : 'Có lỗi xảy ra', variant: 'destructive' });
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle className="text-base">Bảo trì</CardTitle></CardHeader><CardContent><div className="flex items-center justify-between rounded-lg border p-4"><div><p className="font-medium text-foreground">Chế độ bảo trì</p><p className="text-sm text-muted-foreground">Khóa truy cập hệ thống cho người dùng thông thường</p></div><Switch checked={form.maintenanceMode} onCheckedChange={(checked) => setForm((f) => ({ ...f, maintenanceMode: checked }))} /></div></CardContent></Card>
