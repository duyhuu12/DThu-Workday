...
export default function ClassesPage() {
  const { classes, faculties, students, addClass, updateClass } = useAppStore();
  const { toast } = useToast();
  const [open, setOpen] = useState(false); const [editing, setEditing] = useState<Class | null>(null);
  ...
  function openEdit(c: Class) { setEditing(c); setForm({ name: c.name, code: c.code, facultyId: c.facultyId, schoolYear: c.schoolYear }); setOpen(true); }
  async function handleSave() {
    if (!form.name.trim() || !form.code.trim()) { toast({ title: 'Vui lòng điền tên và mã lớp', variant: 'destructive' }); return; }
    if (editing) { await updateClass(editing.id, form); toast({ title: 'Đã cập nhật lớp' }); }
    else { await addClass(form); toast({ title: 'Đã thêm lớp' }); }
    setOpen(false);
  }
  const columns: Column<Class>[] = [
    ...
    { key: 'action', header: '', render: (c) => <Button size="sm" variant="ghost" onClick={() => openEdit(c)}><Pencil className="h-4 w-4" /></Button> },
  ];
