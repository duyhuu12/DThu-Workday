...
export default function WorkCreditsPage() {
  ...
  const totalEarned = myCredits.filter((c) => c.status === 'recorded').reduce((s, c) => s + c.creditValue, 0);
  const totalPending = myCredits.filter((c) => c.status === 'pending').reduce((s, c) => s + c.creditValue, 0);
  const required = student?.requiredWorkdays ?? settings.defaultRequiredWorkdays; const pct = Math.min(100, Math.round((totalEarned / required) * 100));
