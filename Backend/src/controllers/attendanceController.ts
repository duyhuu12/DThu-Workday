...
export async function getSession(req: AuthRequest, res: Response) {
  ...
  try {
    res.status(200).json({
      success: true,
      data: await attendanceService.getAttendanceSession(eventId, req.user.role, req.user.id),
    });
  } catch (error) {
    handleError(res, error, 'Không thể tải phiên điểm danh');
  }
}
...
export async function startSession(req: AuthRequest, res: Response) {
  ...
  try {
    res.status(200).json({
      success: true,
      message: 'Đã bắt đầu phiên điểm danh',
      data: await attendanceService.startAttendanceSession(
        eventId,
        req.body.mode,
        req.user.role,
        req.user.id,
    ...
    handleError(res, error, 'Không thể bắt đầu phiên điểm danh');
  }
}
...
export async function stopSession(req: AuthRequest, res: Response) {
  ...
  try {
    res.status(200).json({
      ...
      data: await attendanceService.stopAttendanceSession(eventId, req.user.role, req.user.id),
    });
  } catch (error) {
    handleError(res, error, 'Không thể kết thúc phiên điểm danh');
  }
}
