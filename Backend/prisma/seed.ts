...
async function main() {
  ...
  const userSecondStudent = await prisma.user.create({
    data: {
      email: 'student2@dthu.edu.vn',
  ...
  const studentSecond = await prisma.student.create({
    data: {
      userId: userSecondStudent.id,
  ...
  const dbStudents = [studentAn, studentSecond];
