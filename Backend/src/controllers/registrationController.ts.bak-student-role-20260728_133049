...
// 1. Get all registrations (with filters for student, event, status)
...
// 2. Register for an event (Student only)
...
// 3. Cancel registration (Student only)
...
// 4. Approve/Reject Registration (Organizer/Admin only)
export async function updateRegistrationStatus(req: AuthRequest, res: Response) {
  ...
  try {
    const data = await registrationService.approveOrReject(regId, status, rejectionReason, req.user.id, req.user.role);
    res.status(200).json({
      success: true,
