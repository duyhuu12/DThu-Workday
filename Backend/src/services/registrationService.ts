...
export async function createRegistration(
  ...
  return prisma.$transaction(async (tx: any) => {
    ...
    const activeEventRegistrationCount = await tx.registration.count({
      where: {
        eventId,
    ...
    if (activeEventRegistrationCount >= event.maxCapacity) {
    ...
    await tx.workEvent.update({
      where: { id: eventId },
      data: { registeredCount: activeEventRegistrationCount + 1 },
    });
    ...
    await tx.notification.create({
      data: {
        ...
        message: `Bạn đã đăng ký thành công sự kiện "${event.name}".`,
