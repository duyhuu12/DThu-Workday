...
export async function updateCreditStatus(creditId: number, status: string, actorId: number) {
  ...
  if (![CreditStatus.RECORDED, CreditStatus.REJECTED, CreditStatus.PENDING].includes(nextStatus)) {
  ...
  const updated = await prisma.$transaction(async (tx: any) => {
    ...
    await tx.notification.create({
      data: {
        ...
        message: `Ngày công của sự kiện "${existing.event.name}" đã chuyển sang trạng thái ${nextStatus.toLowerCase()}.`,
        link: '/student/work-credits',
      },
    });
    await tx.activityLog.create({
      data: {
        ...
        affectedItem: `${existing.student.fullName} - ${existing.event.name}`,
