...
export default function FacultiesPage() {
  ...
  function openAdd() { setEditing(null); setForm({ name: '', code: '' }); setOpen(true); }
  function openEdit(item: Faculty) { setEditing(item); setForm({ name: item.name, code: item.code }); setOpen(true); }
  async function handleSave() {
    if (!form.name.trim() || !form.code.trim()) { toast({ title: 'Vui lòng nhập tên và mã khoa', variant: 'destructive' }); return; }
    setSaving(true);
    try {
