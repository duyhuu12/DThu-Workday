...
export default function WorkEventsPage() {
  ...
  const filtered = useMemo(() => events.filter((e) => {
    ...
    if (faculty !== 'all' && e.eligibleFacultyIds.length > 0 && !e.eligibleFacultyIds.includes(faculty)) return false;
  ...
  const columns: Column<WorkEvent>[] = [
    ...
    { key: 'capacity', header: 'Đăng ký', sortable: true, sortValue: (e) => e.registeredCount, render: (e) => <div className="w-28 space-y-1"><div className="text-xs">{e.registeredCount}/{e.maxCapacity}</div><Progress value={e.maxCapacity > 0 ? (e.registeredCount / e.maxCapacity) * 100 : 0} className="h-1.5" /></div> },
